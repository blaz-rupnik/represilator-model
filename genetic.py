...
def calculate_fitness_for_model(results):
...
def calculate_results_for_model(model, nA, nB, nC):
    new = []
    new_amp = []
    new_per = []
...
def calculate_results_for_generation(model, population):
    ...
    for pop in population:
        specimen_result = calculate_results_for_model(model, pop[0], pop[1], pop[2])
        specimen_fitness = calculate_fitness_for_model(specimen_result)
...
for generation in range(num_generations):
    ...
    start = time.time()
    population_fitness = calculate_results_for_generation(libs.extended_model_1, new_population)
